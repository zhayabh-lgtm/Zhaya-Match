-- Migration: Zhaya Match Clean Database Schema
-- Cleanup old legacy tables if present
DROP TABLE IF EXISTS widget_logs CASCADE;
DROP TABLE IF EXISTS recommendation_events CASCADE;
DROP TABLE IF EXISTS widget_settings CASCADE;
DROP TABLE IF EXISTS product_sizes CASCADE;
DROP TABLE IF EXISTS products CASCADE;
DROP TABLE IF EXISTS size_template_rows CASCADE;
...
DROP TRIGGER IF EXISTS tr_product_types_updated_at ON product_types;
CREATE TRIGGER tr_product_types_updated_at
BEFORE UPDATE ON product_types
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

-- 2. Table: popup_settings
...
DROP TRIGGER IF EXISTS tr_popup_settings_updated_at ON popup_settings;
CREATE TRIGGER tr_popup_settings_updated_at
BEFORE UPDATE ON popup_settings
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

-- 3. Table: text_settings
...
DROP TRIGGER IF EXISTS tr_text_settings_updated_at ON text_settings;
CREATE TRIGGER tr_text_settings_updated_at
BEFORE UPDATE ON text_settings
FOR EACH ROW
EXECUTE FUNCTION update_updated_at_column();

-- 4. Table: measurement_guides
...
DROP TRIGGER IF EXISTS tr_measurement_guides_updated_at ON measurement_guides;
...
DROP TRIGGER IF EXISTS tr_app_settings_updated_at ON app_settings;
...
ALTER TABLE text_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE measurement_guides ENABLE ROW LEVEL SECURITY;
ALTER TABLE app_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE media_assets ENABLE ROW LEVEL SECURITY;

-- Public read policies (visitor / widget access)
CREATE POLICY "Public read active product_types" ON product_types FOR SELECT USING (active = true);
CREATE POLICY "Public read popup_settings" ON popup_settings FOR SELECT USING (true);
CREATE POLICY "Public read text_settings" ON text_settings FOR SELECT USING (true);
CREATE POLICY "Public read active measurement_guides" ON measurement_guides FOR SELECT USING (active = true);
...
-- Authenticated full access policies (Admin access)
CREATE POLICY "Authenticated full access product_types" ON product_types FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated full access popup_settings" ON popup_settings FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated full access text_settings" ON text_settings FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated full access measurement_guides" ON measurement_guides FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated full access app_settings" ON app_settings FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Authenticated full access media_assets" ON media_assets FOR ALL USING (auth.role() = 'authenticated');

-- Storage Policies for bucket zhaya-match-media
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Public Read zhaya-match-media'
  ) THEN
    CREATE POLICY "Public Read zhaya-match-media" ON storage.objects FOR SELECT USING (bucket_id = 'zhaya-match-media');
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated Insert zhaya-match-media'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated Update zhaya-match-media'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Authenticated Delete zhaya-match-media'
  ) THEN
...
-- INITIAL SEED DATA
INSERT INTO product_types (id, name, active, sort_order, measurements, sizes) VALUES
...
INSERT INTO popup_settings (settings, version) VALUES
('{
  "showLogo": true,
  "logoSize": 24,
  "logoPosition": "center",
...
INSERT INTO text_settings (settings) VALUES
('{
  "buttonText": "Descubra seu tamanho",
...
INSERT INTO measurement_guides (measurement_key, label, title, description) VALUES
...
INSERT INTO app_settings (enabled, test_mode, widget_url) VALUES
(true, false, '/widget.js');
