                ...
                <section className="border border-neutral-200 rounded-xl bg-neutral-950 text-white overflow-hidden">
                  ...
                  <div className="relative min-h-[520px] p-6 flex items-center justify-center overflow-hidden" style={{ backgroundColor: draft.backgroundColor, color: draft.textColor }}>
                    ...
                    <div className="relative z-10 w-full max-w-sm text-center">
                      {draft.logoUrl && <img src={draft.logoUrl} alt="" className={`relative left-1/2 -translate-x-1/2 w-[calc(100%+2rem)] max-w-none h-auto max-h-32 object-contain ${draft.title.trim() ? 'mb-7' : (draft.eyebrow || draft.subtitle) ? 'mb-2' : 'mb-0'}`} />}
                      ...
                      {draft.timerEnabled && <div className={draft.title.trim() ? 'mt-7' : (draft.eyebrow || draft.subtitle) ? 'mt-2' : 'mt-0'} style={{ color: draft.timerColor }}><div className="text-[9px] uppercase tracking-[0.24em] opacity-70 leading-none">{draft.timerLabel}</div><div className="text-[2rem] leading-none font-black mt-0.5">50 minutos</div></div>}
                      {(draft.showRemaining || draft.showMaxUnlocks) && draft.maxUnlocks && <div className={`${draft.timerEnabled ? (draft.title.trim() ? 'mt-7' : 'mt-3') : (draft.title.trim() ? 'mt-7' : 'mt-2')} mb-2 text-[9px] uppercase tracking-[0.14em] font-bold opacity-60`}>{draft.showRemaining ? `${Math.max(0, draft.maxUnlocks - (draft.totalUnlocks || 0))} cupons restantes${draft.showMaxUnlocks ? ` de ${draft.maxUnlocks}` : ''}` : `Limite de ${draft.maxUnlocks} cupons`}</div>}
                      <button type="button" className={`w-full ${!(draft.showRemaining || draft.showMaxUnlocks) ? (draft.title.trim() ? (draft.timerEnabled ? 'mt-7' : 'mt-7') : draft.timerEnabled ? 'mt-3' : (draft.eyebrow || draft.subtitle) ? 'mt-3' : 'mt-1') : ''} py-4 rounded-lg text-xs font-black uppercase flex items-center justify-center gap-2`} style={{ backgroundColor: draft.buttonBackgroundColor, color: draft.buttonTextColor }}><LockKeyhole className="w-4 h-4" />{draft.unlockButtonText}</button>
                    </div>
                  </div>
                </section>
              </div>
